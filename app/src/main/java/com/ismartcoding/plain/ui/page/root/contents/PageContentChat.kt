...
fun PageContentChat(
    ...
    channelVM: ChannelViewModel, paddingValues: PaddingValues, currentPage: Int,
) {
    ...
    PageContentChatEffects(peerVM, scope, isAppInForeground, isPageVisible, isScreenOn, currentPage)
    ...
    PullToRefresh(modifier = Modifier.fillMaxSize().padding(top = paddingValues.calculateTopPadding()), refreshLayoutState = refreshState) {
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            item { TopSpace() }
            item { if (!webEnabled) PAlert(title = stringResource(id = R.string.attention), description = stringResource(id = R.string.web_service_required_for_chat), AlertType.WARNING) { PMiniOutlineButton(label = stringResource(R.string.enable_web_service), click = { mainVM.enableHttpServer(context, true) }) } }
