...
internal fun PageContentChatEffects(
    ...
    LaunchedEffect(Unit) {
        while (isActive) {
            ...
            val interval = if (isCurrentTab && isPageVisible.value && isAppInForeground.value && isScreenOn.value) 5000L else 15000L
            LogCat.d("Discovery interval: ${interval}ms (currentTab: $isCurrentTab [visible: ${isPageVisible.value}, foreground: ${isAppInForeground.value}, screenOn: ${isScreenOn.value})")
            delay(interval)
        }
    }
}
